...
def train_test_compose(grad_type):
    ...
    test2_1 = test_model(f2, get_reader('ccpp1.csv'))
    test2_2 = test_model(f2, get_reader('ccpp2.csv'))
    print('Trained with D2, Tested with D1: ' + str(test2_1) + ', D2: ' + str(test2_2) )

    f = compose(f1,f2, grad_type)
...
train_test(Model.GradType.SGD)
print('Train with raw 2nd moment SGD')
train_test(Model.GradType.MOM)
print('Train with momentum and variance')
train_test(Model.GradType.VAR)

print('Compose with SGD')
train_test_compose(Model.GradType.SGD)
print('Compose with raw 2nd moment SGD')
train_test_compose(Model.GradType.MOM)
print('Compose with momentum and variance')
train_test_compose(Model.GradType.VAR)