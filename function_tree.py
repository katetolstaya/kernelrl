...
class KernelRepresentation(object):
	...
	new_nodes = list()
        ...
        if len(neighbors) > 0:
            ...
            temp_nodes = (np.where(self.new_idxs)[0][np.where(temp < lindep_thresh)]).tolist()
            new_removed = (np.where(self.new_idxs)[0][np.where(temp >= lindep_thresh)]).tolist()
        else:
            temp_nodes = np.where(self.new_idxs)[0].tolist()
            new_removed = list()
        #neighbors.extend(new_nodes)

	if len(temp_nodes)>0:
	    #new_nodes = list()
	    new_nodes_temp = np.full((len(temp_nodes),), False, dtype=bool)

	    KDD_temp = self.kernel.f(self.D[temp_nodes, :], self.D[temp_nodes, :]) - np.eye(len(temp_nodes))
	    new_nodes_temp[0] = True
	    for i in range(1,len(temp_nodes)):
		if np.max(KDD_temp[i,new_nodes_temp]) < lindep_thresh:
		    new_nodes_temp[i] = True
		    new_nodes.append(temp_nodes[i])
		else:
		    new_removed.append(temp_nodes[i])

        #print str(len(new_nodes)) + "/" + str(len(new_removed) )
