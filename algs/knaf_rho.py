...
class KNAFModel(object):
    def __init__(self, stateCount, actionCount, config):
        ...
        self.vpln = KernelRepresentation(stateCount, self.dim_a+1, config)
    ...
    def model_error(self):
        return 0.5 * self.lossL * self.vpln.normsq()
    ...
    def predictOne(self, s):  # Predict the Q function values for a single state.
        return self.vpln(np.reshape(s,(1,-1)))
    ...
    def train(self, step, sample):
        ...
        # Gradient step
        self.vpln.shrink(1. - self.lossL)

        # V gradient
        W = np.zeros((self.dim_a+1,))
        ...
        if self.dim_p > 1:
            ...
            W[2] = lgrad_temp  * self.eta_l.value

        W[3] = - (1/(10*np.abs(delta)) - rho) / delta
        ...
        if np.abs(delta) > 50 and False:
            ...
            print (lmat)
            print (delta)
        self.vpln.append(np.array(s), - delta * np.reshape(W, (1, -1)))
        # Prune

        #if step % 10 == 0 :
        self.vpln.prune(self.eps)

        modelOrder_ = len(self.vpln.D)
        # Compute new error
        loss = 0.5 * self.bellman_error(s, a, r, s_) ** 2  # + self.model_error()
        return (float(loss), rho ) #float(modelOrder_))  # ==================================================
