...
class KQLearningAltModel(object):
...
class KQLearningAltAgent(object):
    def __init__(self, env, config):
        ...
        # Initialize model
        self.model = KQLearningAltModel(self.stateCount, self.actionCount, config)
    ...
    def act(self, s, stochastic=True):
        ...
        if stochastic and (random.random() < self.epsilon.value):
            ...
            self.model.last_rand = True
        else:
            a = self.model.Q.argmax(s)
            self.model.last_rand = False
