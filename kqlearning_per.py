...
class KQLearningModel(object):
    ...
    def train(self, step, x, x_, nonterminal, delta, gamma):
        self.eta.step(step)
        self.beta.step(step)
        # Running average of TAD error
        self.y.resize((len(delta),1), refcheck=False)
        ...
        W = np.zeros((len(X), 1))
        W[:len(x)] = self.eta.value * self.y
        W[len(x):] = -self.eta.value * gamma * self.y[nonterminal]
        self.Q.append(X, W)
        # Prune
...
class KQLearningAgentPER(object):
    ...
    def _getStates(self, batch):
        ...
        for i, (_,e) in enumerate(batch):
            if e[3] is not None:
                ...
                # x_[i] = np.hstack((e[3],a_))
                nonterminal.append(i)
        # x_ = np.array([no_state if e[3] is None else
        #     np.hstack((e[3],self.model.maximizeOne(e[3])))
        #     for (_,e) in batch])
        # nonterminal = [i for i, (_,e) in enumerate(batch) if e[3]]
    ...
    def act(self, s, stochastic=True):
        ...
        if stochastic and (random.random() < self.epsilon.value):
            ...
            a = self.model.maximizeOne(s)
            return np.clip(a, self.min_act, self.max_act)

    def observe(self, sample):
        error = self._computeError(*self._getStates([(0, sample)]))
        self.memory.add(sample, (error[0] + self.eps) ** self.alpha)
        self.steps += 1
    ...
    def improve(self):
        ...
        for (idx, _), delta in zip(batch, error):
            self.memory.update(idx, (delta + self.eps) ** self.alpha)
        # update model
